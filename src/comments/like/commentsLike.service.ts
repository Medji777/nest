...
import {CommentsLikeDocument} from "./commentsLike.schema";
...
export class CommentsLikeService {
    ...
    async update(likeInfo: LikesInfo, myStatus: LikeStatus): Promise<void> {
        const like = await this.commentsLikeRepository.findByUserIdAndCommentId(likeInfo.userId, likeInfo.commentId);
        if(!like){
