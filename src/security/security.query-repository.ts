...
export class SecurityQueryRepository {
    constructor(@InjectModel(Security.name) private SecurityModel: SecurityModelType) {}
    async getAllActiveSessions(userId: string): Promise<Array<DeviceViewModel>>{
        return this.SecurityModel
            .find({userId},{expiredTokenDate: 0, userId: 0, ...projectionInit})
            .lean()
    }
    async findSession(userId: string, deviceId: string): Promise<DeviceModel | null>{
        return this.SecurityModel.findOne({userId,deviceId},projectionInit).lean()
    }
